...
app.get("/api/activeResource", (req, res )=>{
    ...
    if (!activeResource) {
      res.status(422).send("no active resource found")
    }
...
app.patch("/api/resources/:id", (req, res) => {
  ...
  if (resources[index].status === "completed") {
    return res.status(422).send("Cannot activate this resource since it has already been completed");
  }
  ...
  if(req.body.status === "active") {
    if(activeResource) {
      return res.status(422).send("There is currently another active resource");
    }
...
app.listen(PORT, ()=> {console.log("like a champ")})